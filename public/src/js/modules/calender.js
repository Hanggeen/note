	...
	C.prototype = {
		init : function(){
			...
			t.nt.setFullYear(t.s.date[0]);
		  	...
		  	t.selected = new Date();
		  	t.selected.setFullYear(t.nt.getFullYear());
		  	t.selected.setMonth(t.nt.getMonth());
		  	t.selected.setDate(t.nt.getDate());


		  	t.set = function (p) {
				if ( typeof p == 'string') {
					t.nt = new Date(p.replace(/-/g, "/"));
		  	...
		  	t.setMin = function (arr) {
		  		t.s.min = arr;
		  	}
		  	t.setMax = function (arr) {
		  		t.s.max = arr;
		  	}
		...
		hide : function(set){
			...
			if(set){
				t.cb.call(t.dom,t.format( t.nt.getFullYear()+'/'+ (t.nt.getMonth()+1)+'/'+ t.day+' '+new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds(),t.s.format));
			  	...
			  	t.nt.setFullYear(t.selected.getFullYear());
			  	t.nt.setMonth(t.selected.getMonth());
			  	t.nt.setDate(t.selected.getDate());
			  	t.day = t.selected.getDate();
			}
		...
		createDay : function(n){
			var t = this, 
				y = t.nt.getFullYear(),
				m = (t.nt.getMonth())+1,
				oy = t.selected.getFullYear(),
				om = (t.selected.getMonth())+1,
				od = t.selected.getDate(),
				c = t.s;
			...
			function test (arr, minArr, maxArr) {
				var target = new Date((arr[0]+'-'+arr[1]+'-'+arr[2]).replace(/-/g, "/")),
					min = new Date((minArr[0]+'-'+minArr[1]+'-'+minArr[2]).replace(/-/g, "/")),
					max = new Date((maxArr[0]+'-'+maxArr[1]+'-'+maxArr[2]).replace(/-/g, "/"));
			...
			for(i=1;i<length+1;i++){
				html += '<div  data-n='+i+' class="calender-cell '+ (test([y,m,i],c.min,c.max)?'':' calender-cell-dark ') +(testActive(y,m,i) ? 'active' :'') +'">' + i + '</div>';
			}
